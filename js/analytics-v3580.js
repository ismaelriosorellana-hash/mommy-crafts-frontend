...
(function () {
    ...
    function track(name, params = {}) {
        ...
        if (!eventName || !state.enabled) return;
    ...
    function loadGoogleAnalytics() {
        ...
        if (!GA4_RE.test(id) || state.gaLoaded) return;
    ...
    function loadClarity() {
        ...
        if (!CLARITY_RE.test(id) || state.clarityLoaded) return;
    ...
    window.MommyAnalytics = Object.freeze({
        ...
        trackCheckoutStart,
        trackPurchase,
        getState: () => ({ ...state, pending: state.pending.length })
    });
